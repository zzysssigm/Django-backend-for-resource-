...
最麻烦的通知功能/回复功能/发帖功能，慢慢琢磨吧，感觉好复杂啊，目前写的是：发replytoreply会带一个[@{username}]，可以检查并通知；post直接通知article.author；reply同理，直接通知post.poster；
然后现在还缺一个跳转功能，添加下锚点即可，交给前端了....反正接口写差不多了

比较重要的course类，把这个写好就基本成型了；course的增加/修改/删除；评论/帖子的删除；course不能重名；
